...
const TestFetch = () => {
	const [
		fullHistoricalClosingPrices,
		setFullHistoricalClosingPrices
	] = useState([]);
	...
	useEffect(() => {
		(async () => {
			...
			console.log(data);
			setFullHistoricalClosingPrices([...data.historical]); // Complete historical API fetch
			console.log(fullHistoricalClosingPrices);
			fullHistoricalClosingPrices.map((day, i) => {
				i < 30 ? rolling30DayClosingPrices.push(day.close) : '';
			});
			fullHistoricalClosingPrices.map((day, i) => {
				i < 30 ? console.log(day.close) : '';
			});
			...
			{rolling30DayClosing.length ? (
				<div id="line-chart">
					<Line // Per documentation you are supposed to have a key value pair here such as type: 'bar'; However, because we imported { Bar }  in code above from the 'react-chartjs-2' library we only need to create the <Bar Component here and close it at the end after we input all the lines of code between it. This is how it works with React.
						data={{
							labels: months, // Sets the x-axis labels that goes horizontally across screen. Place '' in the spot of the array where you don't want to show the label.
							datasets: [
								{
