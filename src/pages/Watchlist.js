...
*/
	...
	const sendToDB = async newSymbol => {
		try {
			...
			// Making the API request to Financial Modeling Prep to check to see if symbol entered in by user is valid. If NOT, then there is a notification sent to user that entry is invalid. In later iterations I can refine code to take the data it receives back here and not fetch again if data entered in by user is valid. Essentially right now it is fetching the data twice which is unecessary (for time's sake needed to move on and most importantly it works right now)
			...
			const dataAPIValidStockFetch = await APIValidStockFetch.json(); // Awaiting the response from Financial Modeling Prep API and converting the json object sent back into a Javascript object through use of the .json() method so that we can use this to get data about our stocks.
	...
	return (
		<div id="watchlist-wrapper">
			...
			<div className="watchlist-overallContainer">
				...
				<h3
					className={
						validTicker ? 'invalid-entry-hidden' : 'invalid-entry-message'
					}
				>
					Invalid Ticker Symbol Entered
				</h3>
