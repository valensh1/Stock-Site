...
const FinancialAPIKey = process.env.FINANCIAL_API_KEY;
console.log(FinancialAPIKey);
...
export default function App(props) {
	...
	}, [DBWatchlist]); // Once useEffect above is executed upon initial rendering of screen the DBWatchlist is updated which then causes this useEffect to execute which then pulls API data for stocks listed in our database on user's watchlist
	...
	const sendToDB = async data => {
		...
		const watchlistData = {
			...
			symbol: tickerSymbol,
			lastPrice: data['Time Series (Daily)']['2021-02-26']['5. adjusted close'] // WOULD NEED TO ADJUST DATE HERE IF SOMETHING WRONG WITH DATE FOR SENDING TO DATABASE
		};
		try {
